...
app.post("/update", async (req, res) => {
  ...
  const update = {
    properties: {
      ...
      years_to_study: req.body.years_to_study,
    },
  };

  try {
    const response = await axios.post(customObjUrl, update, { headers });
    const data = response.data.results;
    if (response.status === 200 || response.status === 201) {
      res.redirect("/");
    } else {
      throw new Error(`Object could not be created ${response.status}`);
    }
  } catch (error) {
    console.error(error);
  }
